...
export default function WaterObservations({ locationId, api }) {
  ...
  return <section className="water-observations" aria-label="Käyttäjien vesipistehavainnot">
    <h3>Retkeilijöiden vesipistetiedot</h3>
    ...
    {loading ? <p role="status">Ladataan havaintoja…</p> : loadError ? <div role="alert"><p>{loadError}</p><button type="button" className="secondary" onClick={() => { setLoadError(''); setLoading(true); setReload(n => n+1); }}>Yritä uudelleen</button></div> : <>
      {!observations.length && <p className="source-note">Ei vielä käyttäjien vesipistehavaintoja tästä kohteesta.</p>}
      {observations.map(item => <article key={item.id} className="observation water-observation">
        <span className="type-tag">Käyttäjän ilmoittama vesipiste</span>
      ...
      <button type="button" className="secondary" aria-expanded={open} aria-controls="water-form" onClick={() => setOpen(value => !value)} disabled={saving}>
        {open ? 'Sulje vesipistelomake' : observations.length ? 'Lisää vesipistehavainto' : 'Ilmoita vesipisteestä'}
      </button>
      {open && <form id="water-form" className="report-form" onSubmit={submit}>
        <h4>Vesipiste tämän taukopaikan yhteydessä</h4>
        <label htmlFor="water-kind">Vesipisteen tyyppi</label>
        <select id="water-kind" required value={draft.kind} disabled={saving} onChange={e => update({ kind: e.target.value })}>
          <option value="">Valitse tyyppi</option>{Object.entries(kinds).map(([key,name]) => <option key={key} value={key}>{name}</option>)}
        </select>
        <label htmlFor="water-directions">Missä vesipiste sijaitsee?</label>
        <textarea id="water-directions" required maxLength={1000} value={draft.directions} disabled={saving} onChange={e => update({ directions: e.target.value })} placeholder="Esimerkiksi: kaivo noin 50 metriä laavulta, käymälälle johtavan polun vieressä." />
