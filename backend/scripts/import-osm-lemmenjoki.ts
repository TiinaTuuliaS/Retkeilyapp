...
const scope = { lemmenjoki: ['4176627279','4181473143','4181554783'] };
const read = (path: string) => JSON.parse(readFileSync(resolve(backendRoot, '..', path), 'utf8'));
function preview() {
  const data = read('data-preview/osm-parks/lemmenjoki-proposal.geojson');
  const review = { proposed: data.features.map(f => ({ ...f.properties, id:f.id, geometry:f.geometry, park:'lemmenjoki' })) };
  assert.equal(review.proposed.length, 3);
  ...
  return review.proposed.map(p => {
    ...
    assert.equal(p.tags.leisure, 'firepit');
    const type = 'fireplace';
