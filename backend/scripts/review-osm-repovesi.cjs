...
const all = read(path.join(base,'repovesi.geojson')).features;
const inside = all.filter(f => f.properties.classification === 'inside');
const lipas = read(path.join(root,'data-preview/lipas-repovesi/sites.geojson')).features;
...
const proposed = inside.filter(f => ['osm:node:1350040125','osm:node:2185581285'].includes(f.id));
assert.equal(proposed.length,2);
for (const f of proposed) {
  assert.equal(f.properties.positionKind,'osm-node');
  assert.ok(f.properties.tags.tourism === 'picnic_site' || f.properties.tags.amenity === 'drinking_water');
...
fs.writeFileSync(path.join(base,'repovesi-review.json'),JSON.stringify({checkedAt:new Date().toISOString(),
...
fs.writeFileSync(path.join(base,'repovesi-proposal.geojson'),JSON.stringify({type:'FeatureCollection',
