...
const scope = { repovesi: ['1350040125','2185581285'] };
const read = (path: string) => JSON.parse(readFileSync(resolve(backendRoot, '..', path), 'utf8'));
function preview() {
  const data = read('data-preview/osm-parks/repovesi-proposal.geojson');
  const review = { proposed: data.features.map(f => ({ ...f.properties, id:f.id, geometry:f.geometry, park:'repovesi' })) };
  assert.equal(review.proposed.length, 2);
  ...
  return review.proposed.map(p => {
    ...
    assert.ok(p.tags.tourism === 'picnic_site' || p.tags.amenity === 'drinking_water');
    const type = p.tags.amenity === 'drinking_water' ? 'water' : 'rest_area';
...
async function main() {
  ...
  try {
    ...
    await client.query('SELECT pg_advisory_xact_lock(73487,73859)');
