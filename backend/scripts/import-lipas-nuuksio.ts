...
function readPreview(): PreviewFeature[] {
  ...
  const raw = JSON.parse(readFileSync(resolve(backendRoot,'../data-preview/lipas-nuuksio/raw.json'),'utf8'));
  return ids.map(id => {
...
async function main() {
  ...
  try {
    ...
    await client.query('SELECT pg_advisory_xact_lock(73487,73860)');
    await client.query(readFileSync(resolve(backendRoot, 'migrations/001-location-sources.sql'), 'utf8'));
    const boundary = JSON.parse(readFileSync(resolve(backendRoot,'../data-preview/lipas-nuuksio/boundary.geojson'),'utf8'));
    assert.equal(boundary.features[0].properties.LsAlueTunn,'KPU010030');
    ...
    if (verify) {
      const observedSite = results.find(r => r.sourceId === '73487')!;
      ...
      const source = await client.query("SELECT metadata->>'source_status' AS status FROM public.location_sources WHERE source='lipas' AND source_id='73487'");
      assert.equal(source.rows[0].status,features.find(f => f.properties.source_id === '73487')!.properties.source_status);
