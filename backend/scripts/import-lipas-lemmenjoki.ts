...
function readPreview(): PreviewFeature[] {
  const ids = [73872,73951,74120];
  const raw = JSON.parse(readFileSync(resolve(backendRoot,'../data-preview/lipas-lemmenjoki/raw.json'),'utf8'));
  return ids.map(id => {
    ...
    return { type:'Feature', id:`lipas:sports-sites:${id}`, geometry:fs[0].geometry,
      properties: {name:site.name,type:id === 73951 ? 'lean_to' : 'weather_shelter',source:'lipas',source_collection:'sports-sites',source_id:String(id),
...
async function main() {
  ...
  try {
    ...
    await client.query('SELECT pg_advisory_xact_lock(73487,73855)');
    await client.query(readFileSync(resolve(backendRoot, 'migrations/001-location-sources.sql'), 'utf8'));
    const boundary = JSON.parse(readFileSync(resolve(backendRoot,'../data-preview/lipas-lemmenjoki/boundary.geojson'),'utf8'));
    assert.equal(boundary.features[0].properties.LsAlueTunn,'KPU120024');
    ...
    if (verify) {
      const observedSite = results.find(r => r.sourceId === '73872')!;
      ...
      const source = await client.query("SELECT metadata->>'source_status' AS status FROM public.location_sources WHERE source='lipas' AND source_id='73872'");
      assert.equal(source.rows[0].status,features.find(f => f.properties.source_id === '73872')!.properties.source_status);
