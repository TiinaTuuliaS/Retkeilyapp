...
const { polygonPosition, polygons } = require('./preview-lipas-helvetinjarvi.cjs');
const fs = require('node:fs/promises');
const path = require('node:path');
const assert = require('node:assert/strict');
async function main() {
  const startedAt = new Date().toISOString();
  const dir = path.resolve(__dirname, '../../data-preview/lipas-helvetinjarvi/checks', startedAt.replace(/[:.]/g,'-'));
  await fs.mkdir(dir, { recursive: true });
  const sources = {
    sports: 'https://api.lipas.fi/v2/sports-sites?city-codes=702&statuses=active,out-of-service-temporarily&page-size=100',
