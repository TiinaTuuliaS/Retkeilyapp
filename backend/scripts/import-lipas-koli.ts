...
function readPreview(): PreviewFeature[] {
  const ids = [509040,509042,511172];
  const raw = JSON.parse(readFileSync(resolve(backendRoot,'../data-preview/lipas-koli/raw.json'),'utf8'));
  return ids.map(id => {
    ...
    return { type:'Feature', id:`lipas:sports-sites:${id}`, geometry:fs[0].geometry,
      properties: {name:site.name,type:'fireplace',source:'lipas',source_collection:'sports-sites',source_id:String(id),
        source_type_code:206,source_status:site.status,source_url:`https://api.lipas.fi/v2/sports-sites/${id}`,
        source_event_date:site['event-date'],fetched_at:record.fetchedAt,
        source_comment:site.comment ?? null,
  ...
  if (existing.rowCount) {
    ...
    const update = await client.query(
      `UPDATE public.locations SET name=$1, type=$2, longitude=$3, latitude=$4,
       geom=ST_SetSRID(ST_MakePoint($3,$4),4326),description=$6 WHERE id=$5 RETURNING id`,
      [p.name, p.type, longitude, latitude, locationId, p.source_comment],
    );
    ...
    const inserted = await client.query(
      `INSERT INTO public.locations (name,type,longitude,latitude,geom,description)
       VALUES ($1,$2,$3,$4,ST_SetSRID(ST_MakePoint($3,$4),4326),$5) RETURNING id`,
      [p.name, p.type, longitude, latitude, p.source_comment],
    );
...
async function main() {
  ...
  try {
    ...
    await client.query('SELECT pg_advisory_xact_lock(73487,73857)');
    await client.query(readFileSync(resolve(backendRoot, 'migrations/001-location-sources.sql'), 'utf8'));
    const boundary = JSON.parse(readFileSync(resolve(backendRoot,'../data-preview/lipas-koli/boundary.geojson'),'utf8'));
    assert.equal(boundary.features[0].properties.LsAlueTunn,'KPU070027');
    ...
    if (verify) {
      const observedSite = results.find(r => r.sourceId === '509040')!;
      ...
      const observation = await usageService.create(observedSite.locationId, {
        status:'in_use',comment:'Rollback-only usage verification',observedOn:'2026-01-01',
      });
      assert.equal(observation.locationId,observedSite.locationId);
      ...
      const retained = await usageService.find(observedSite.locationId);
      assert.ok(retained.some(row => row.id === observation.id && row.status === 'in_use'));
      const source = await client.query("SELECT metadata->>'source_status' AS status FROM public.location_sources WHERE source='lipas' AND source_id='509040'");
      assert.equal(source.rows[0].status,features.find(f => f.properties.source_id === '509040')!.properties.source_status);
