...
function readPreview(): PreviewFeature[] {
  const ids = [73643];
  const raw = JSON.parse(readFileSync(resolve(backendRoot,'../data-preview/lipas-repovesi/raw.json'),'utf8'));
  return ids.map(id => {
...
async function main() {
  ...
  try {
    ...
    await client.query('SELECT pg_advisory_xact_lock(73487,73858)');
    await client.query(readFileSync(resolve(backendRoot, 'migrations/001-location-sources.sql'), 'utf8'));
    const boundary = JSON.parse(readFileSync(resolve(backendRoot,'../data-preview/lipas-repovesi/boundary.geojson'),'utf8'));
    assert.equal(boundary.features[0].properties.LsAlueTunn,'KPU050034');
    ...
    if (verify) {
      const observedSite = results.find(r => r.sourceId === '73643')!;
      ...
      const source = await client.query("SELECT metadata->>'source_status' AS status FROM public.location_sources WHERE source='lipas' AND source_id='73643'");
      assert.equal(source.rows[0].status,features.find(f => f.properties.source_id === '73643')!.properties.source_status);
