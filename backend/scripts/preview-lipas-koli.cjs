...
const dir = path.join(root, 'data-preview/lipas-koli');
...
async function main() {
  ...
  const query = new URLSearchParams({ where: "Nimi LIKE '%Kolin%'", outFields: '*', outSR: '4326', f: 'geojson' });
  ...
  assert.ok(boundary.features[0].properties.Nimi.includes('Kolin'));
