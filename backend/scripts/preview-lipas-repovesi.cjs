...
const dir = path.join(root, 'data-preview/lipas-repovesi');
...
async function main() {
  ...
  const query = new URLSearchParams({ where: "Nimi LIKE '%Repoveden%'", outFields: '*', outSR: '4326', f: 'geojson' });
  const boundary = offline ? JSON.parse(fs.readFileSync(path.join(dir,'boundary.geojson'),'utf8')) : await get(endpoint + '/query?' + query);
  assert.equal(boundary.features?.length, 1, 'Expected one Repovesi boundary');
  assert.ok(boundary.features[0].properties.Nimi.includes('Repoveden'));
  ...
  const cached = offline ? JSON.parse(fs.readFileSync(path.join(dir,'raw.json'),'utf8')) : [];
  const raw = [], features = [];
  for (const id of ids) {
    const url = `https://api.lipas.fi/v2/sports-sites/${id}`;
    const record = cached.find(r => r.site['lipas-id'] === id);
    const site = offline ? record.site : await get(url);
    assert.equal(site['lipas-id'], id);
    raw.push({ fetchedAt: offline ? record.fetchedAt : new Date().toISOString(), url, site });
