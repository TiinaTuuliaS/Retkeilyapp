...
function readPreview(): PreviewFeature[] {
  const ids = [73132, 73411, 73560, 73593, 73595, 73659, 73674, 73683, 73703, 73794, 73856, 73988, 74165, 74229, 74234, 74265, 74362, 74407, 604074, 72957, 72968, 73020, 73066, 73095, 73130, 73349, 73407, 73501, 73675, 73803, 73887, 73926, 73927, 74047, 74385, 74394];
  const raw = JSON.parse(readFileSync(resolve(backendRoot,'../data-preview/lipas-oulanka/raw.json'),'utf8'));
  return ids.map(id => {
    ...
    return { type:'Feature', id:`lipas:sports-sites:${id}`, geometry:fs[0].geometry,
      properties: {name:site.name,type:site.type['type-code'] === 301 ? 'shelter' : site.name.toLocaleLowerCase('fi').includes('taukokatos') ? 'rest_shelter' : 'fireplace',source:'lipas',source_collection:'sports-sites',source_id:String(id),
        source_type_code:site.type['type-code'],source_status:site.status,source_url:`https://api.lipas.fi/v2/sports-sites/${id}`,
...
async function main() {
  ...
  try {
    ...
    await client.query('SELECT pg_advisory_xact_lock(73487,73856)');
    await client.query(readFileSync(resolve(backendRoot, 'migrations/001-location-sources.sql'), 'utf8'));
    const boundary = JSON.parse(readFileSync(resolve(backendRoot,'../data-preview/lipas-oulanka/boundary.geojson'),'utf8'));
    assert.equal(boundary.features[0].properties.LsAlueTunn,'KPU110020');
    ...
    if (verify) {
      const observedSite = results.find(r => r.sourceId === '73132')!;
      ...
      const source = await client.query("SELECT metadata->>'source_status' AS status FROM public.location_sources WHERE source='lipas' AND source_id='73132'");
      assert.equal(source.rows[0].status,features.find(f => f.properties.source_id === '73132')!.properties.source_status);
