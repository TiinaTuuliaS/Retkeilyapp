...
const dir = path.join(root, 'data-preview/lipas-pallas');
...
async function main() {
  ...
  const query = new URLSearchParams({ where: "Nimi LIKE '%Pallas%'", outFields: '*', outSR: '4326', f: 'geojson' });
  const boundary = await get(endpoint + '/query?' + query);
  assert.equal(boundary.features?.length, 1, 'Expected one Pallas boundary');
  assert.ok(boundary.features[0].properties.Nimi.includes('Pallas'));
