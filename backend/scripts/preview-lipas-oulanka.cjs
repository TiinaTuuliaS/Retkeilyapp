...
const dir = path.join(root, 'data-preview/lipas-oulanka');
...
async function main() {
  ...
  const query = new URLSearchParams({ where: "Nimi LIKE '%Oulan%'", outFields: '*', outSR: '4326', f: 'geojson' });
  const boundary = offline ? JSON.parse(fs.readFileSync(path.join(dir,'boundary.geojson'),'utf8')) : await get(endpoint + '/query?' + query);
  assert.equal(boundary.features?.length, 1, 'Expected one Oulan boundary');
  assert.ok(boundary.features[0].properties.Nimi.includes('Oulan'));
