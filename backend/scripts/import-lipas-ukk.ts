...
function readPreview(): PreviewFeature[] {
  const path = resolve(backendRoot, '../data-preview/lipas-ukk/import.geojson');
  const data = JSON.parse(readFileSync(path, 'utf8').replace(/^\uFEFF/, ''));
  assert.equal(data.type, 'FeatureCollection');
  assert.ok(Array.isArray(data.features) && data.features.length === 33,
    'This import requires exactly the 33 reviewed UKK sites.');
  const reviewed = JSON.parse(readFileSync(resolve(backendRoot, '../data-preview/lipas-ukk/sites.geojson'), 'utf8')).features.filter(f => f.properties.park_classification === 'inside');
  const approvedIds = reviewed.map(f => String(f.properties.source_id));
  assert.equal(approvedIds.length, 33);
  const ids = new Set<string>();
  for (const feature of data.features) {
    const p = feature.properties;
    assert.equal(feature.type, 'Feature');
    assert.ok(p && approvedIds.includes(p.source_id));
    ...
    assert.ok([301, 206].includes(p.source_type_code));
    assert.equal(p.type, p.source_type_code === 301 ? 'shelter' : 'fireplace');
    ...
    const original = reviewed.find(f => String(f.properties.source_id) === p.source_id);
    assert.deepEqual(coordinates, original.geometry.coordinates, 'Coordinates changed; recheck park membership.');
    assert.equal(p.source_type_code, original.properties.type_code);
  }
